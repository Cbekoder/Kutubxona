...
def record_info(request, record_id):
...
# Tug’ilgan yilidan kelib chiqib yoshi eng katta bo’lgan 3 ta muallifni chiqaruvchi view/html yozing.
def muallif_info(request, muallif_id):
    if request.user.is_authenticated:
        ...
        return render(request, 'mashq_uchun/muallif_info.html', context)
    return redirect("/login")
def katta_maulliflar(request):
    content = {
        "royxat" : Muallif.objects.order_by("tugilgan_sana")[:3]
    }
    return render(request, "katta_mualliflar.html", content)
# . Kitob soni 10 tadan kichik bo’lgan mualliflarning hamma kitoblarini chiqaruvchi view/html yozing.
def kitob_10_muallif(request):
    content = {
        "kitoblar" : Kitob.objects.filter(muallif__kitoblar_soni__lt= 10)
    }
    return render(request, "kitob10.html", content)
